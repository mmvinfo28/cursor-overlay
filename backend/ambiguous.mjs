...
// Ambiguous hosts no model: the coworker is this process. Its identity is the agent key (ak_…) from
// https://app.ambiguous.ai/developers (or `npx ambiguous`). Set AMBIGUOUS_DRY=1 to log instead of posting.
...
const AKEY = env.AMBIGUOUS_DRY ? (env.AMBIGUOUS_API_KEY || 'dry') : need('AMBIGUOUS_API_KEY');
...
// ---------- Ambiguous REST ----------
async function ambi(method, path, body) {
  if (DRY && method !== 'GET') { log('DRY', method, path, JSON.stringify(body).slice(0, 200)); return { id: 'dry-' + Date.now(), url: `${AMBI}/dry` }; }
  const r = await fetch(`${AMBI}${path}`, {
    method, headers: { Authorization: `Bearer ${AKEY}`, 'Content-Type': 'application/json', Accept: 'application/json' },
...
let channelId = env.AMBIGUOUS_CHANNEL_ID || null;
async function channel() {
  if (channelId) return channelId;
  if (DRY) return (channelId = 'dry-channel');
  const chans = list(await ambi('GET', '/api/channels'));
  const hit = chans.find((c) => (c.name || '').replace(/^#/, '').toLowerCase() === CHANNEL.toLowerCase());
  if (hit) return (channelId = hit.id);
  const made = await ambi('POST', '/api/channels', { name: CHANNEL, type: 'public', description: 'Crewboard — tasks, questions and results from the crew' });
  channelId = made.id || made.data?.id;
  log('created channel', CHANNEL, channelId);
  return channelId;
}
const say = async (content, thread_key) => ambi('POST', `/api/channels/${await channel()}/messages`, thread_key ? { content, thread_key } : { content });
...
async function deliver() {
  const rows = await sb(`tasks?status=eq.done&updated_at=gt.${encodeURIComponent(state.since)}&select=id,title,context,source_app,result,updated_at,worker:workers(name),deliverables(kind,name,url,body)&order=updated_at.asc&limit=20`);
  for (const t of rows || []) {
    ...
    let docUrl = null;
    try {
      const doc = await ambi('POST', '/api/documents', { type: 'doc', title: t.title, content: md, visibility: 'workspace' });
...
async function askHumans() {
  const rows = await sb(`tasks?status=eq.needs-human&select=id,title,events(id,kind,payload,at)&limit=50`);
  for (const t of rows || []) {
    const q = (t.events || []).filter((e) => e.kind === 'needs-human').sort((a, b) => (a.at < b.at ? 1 : -1))[0];
    if (!q || state.asked[t.id] === q.id) continue;
    await say(`❓ **${t.title}** — ${NAME} needs a human: ${q.payload?.question || 'your input'}\n_Reply in this thread to answer._`, `task-${t.id}`);
    state.asked[t.id] = q.id; save(); log('asked', t.title);
  }
  // replies in a task thread → human-answer (thread_key is ours, so we can find it by searching recent messages)
  if (DRY) return;
  for (const [taskId] of Object.entries(state.asked)) {
    let msgs = [];
    try { msgs = list(await ambi('GET', `/api/channels/${await channel()}/messages?limit=30`)); } catch { continue; }
    ...
    await say(`↩ Passed your answer to the worker.`, `task-${taskId}`);
...
// ---------- loop ----------
log(`${NAME} online — Ambiguous ${AMBI} #${CHANNEL} · model ${MODEL} @ ${LLM} · Crewboard ${API}${DRY ? ' · DRY RUN' : ''}`);
let stopping = false;
